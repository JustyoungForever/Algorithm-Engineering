...
int main() {
    ...
    std::ofstream threads_csv("threads_speedup.csv");
    threads_csv << "Threads,QS_Speedup,GNU_Speedup\n";
    std::cout << "thread is :" << omp_get_max_threads() << "\n";
    int max_threads = omp_get_max_threads();
    for (int threads = 1; threads <= max_threads; threads += 1) {
    ...
    std::vector<int64_t> data_copy_parallel = data;  // duplicate data for __gnu_parallel::sort
        ...
        double qs_speedup = time_std_sort / time_min_max_quicksort;
        double gnu_speedup = time_std_sort / time_gnu_parallel;

        threads_csv << threads << "," << qs_speedup << "," << gnu_speedup << "\n";
        std::cout << "thread is :" << threads << "\n";
    }

    threads_csv.close();

    std::ofstream size_csv("array_speedup.csv");
    size_csv << "Size,QS_Speedup,GNU_Speedup\n";
    ...
    for (int64_t size = 10; size <= 1e7; size *= 10) {
        std::vector<int64_t> data(size);
