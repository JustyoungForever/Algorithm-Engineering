...
void calcThresholdMatrix(const cv::Mat& grayImage, const cv::Mat& localMean, const cv::Mat& localStdDev, cv::Mat& threshold) {
    // Calculate the global mean of the grayscale image
    double mg = cv::mean(grayImage)[0];
    cout << "Inside calcThresholdMatrix function" << endl;

    // Initialize the threshold matrix with zeros
    threshold = cv::Mat::zeros(grayImage.size(), CV_64F);

    int blockSize = 24; 

    // Find the minimum and maximum local standard deviation across the entire image
    ...
    // Calculate the range of local standard deviation to avoid repeated calculations
    double sigmaRange = sigmaMax - sigmaMin;

    // Use OpenMP to parallelize the loop for better performance
    ...
    for (int y = 0; y < grayImage.rows; y += blockSize) {
        for (int x = 0; x < grayImage.cols; x += blockSize) {
            ...
            // Determine the actual height and width of the block, considering image boundaries
            ...
            for (int i = y; i < y + blockHeight; ++i) {
                for (int j = x; j < x + blockWidth; ++j) {
                    ...
                    // Retrieve the local mean and standard deviation for the current pixel
                    double mw = localMean.at<double>(i, j);
                    // the local standard deviation not zero to avoid division by zero
                    double sigmaW = std::max(localStdDev.at<double>(i, j), 1e-6);                    

                    // Calculate the adaptive standard deviation factor
                    double sigmaAdaptive = (sigmaW - sigmaMin) / sigmaRange;

                    // Compute the denominator of the threshold calculation formula
                    double denominator = (mg + sigmaW) * (sigmaAdaptive + sigmaW);
                    
                    // Calculate the threshold only if the denominator is not zero
                    if (denominator != 0) {
                        double T = mw - (mw * mw - sigmaW * sigmaW) / denominator;
                        // Assign the calculated threshold to the corresponding pixel in the threshold matrix
...
//Before I chose this method, I tried the following writing methods
// and the writing methods in other literature. In the end I felt this worked best.
